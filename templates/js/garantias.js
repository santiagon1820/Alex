...
window.onload = () => {
  ...
  const emailParam = params.get("correo");
  if (emailParam) {
...
function doLogin() {
  ...
  if (!email || !email.includes("@"))
    return showToast("Ingresa un correo válido", "error");
  userEmail = email;
...
async function loadTickets() {
  ...
  try {
    const res = await fetch(
      `/api/getTicketsByUser?email=${encodeURIComponent(userEmail)}&ticket_type=1`,
    );
...
function renderTickets() {
  ...
  if (!filtered.length) {
    list.innerHTML = `<div class="empty-state">No hay tickets ${activeFilter === "all" ? "" : activeFilter === "open" ? "abiertos" : "cerrados"}</div>`;
    return;
  }
  list.innerHTML = filtered
    .map((t) => {
      ...
      return `
      <div class="ticket-item ${currentTicket?.id === t.id ? "active" : ""}"
...
function connectSSE(ticketId) {
  closeSSE(); // cierra la conexión previa antes de abrir la nueva
  const url = `/api/chat?ticket_id=${ticketId}&user_type=1&email=${encodeURIComponent(userEmail)}`;
        ...
        `
          ...
          <div class="input-area">
            ...
            <div class="input-row">
              ...
              <textarea id="msgInput" placeholder="Escribe tu mensaje…" rows="1"
...
function appendMessage(msg) {
  ...
  const initials = isAgent ? "AG" : msg.user.slice(0, 2).toUpperCase();
  const avatarClass = isAgent ? "avatar-agent" : "avatar-client";
...
async function sendMessage() {
  ...
  if (isUploading()) return; // guardia extra por si acaso
  ...
  try {
    ...
    showToast(e.message, "error");
    // Restaurar mensaje en el input si falló
...
function handleFileSelect(e) {
  Array.from(e.target.files).forEach((file) => {
    ...
    // Bloquear enviar de inmediato antes de renderizar
    const btn = document.getElementById("sendBtn");
...
async function createTicket() {
  ...
  try {
    const res = await fetch(`/api/generateTicket`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email: userEmail, type_ticket: 1, subject }),
    });
    const data = await res.json();
    if (!res.ok) throw new Error(data.Error || "Error al crear ticket");
    showToast(data.message || "Ticket creado", "success");
    closeNewModal();
    loadTickets();
  } catch (e) {
    showToast(e.message, "error");
